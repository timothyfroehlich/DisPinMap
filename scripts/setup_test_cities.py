...
async def setup_test_cities():
    ...
    try:
        with database.get_session() as session:
            ...
            channel_config = session.query(ChannelConfig).filter_by(
                channel_id=CONSOLE_CHANNEL_ID
            ).first()
            
            if not channel_config:
                print(f"📺 Creating channel config for console channel ({CONSOLE_CHANNEL_ID})")
                ...
                print(f"📺 Console channel config already exists")
            ...
            for city_name, latitude, longitude in TEST_CITIES:
                ...
                existing = session.query(MonitoringTarget).filter_by(
                    channel_id=CONSOLE_CHANNEL_ID,
            ...
            all_targets = session.query(MonitoringTarget).filter_by(
                channel_id=CONSOLE_CHANNEL_ID
            ).all()
            ...
            print(f"   python src/local_dev.py")
...
if __name__ == "__main__":
    if not os.path.exists(".env.local"):
        print("❌ .env.local file not found. Please set up local development environment first.")
        sys.exit(1)
