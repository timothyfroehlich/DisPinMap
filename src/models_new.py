...
class MonitoringTarget(Base):
    ...
    display_name = Column(String, nullable=False)  # Always human-readable name for users
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "target_type IN ('location', 'geographic')",
        ...
        UniqueConstraint(
            ...
            name="unique_location_target"
        ),
        UniqueConstraint(
            "channel_id", "latitude", "longitude", "radius_miles",
    ...
    def validate_target_type(self, key, target_type):
        ...
        if target_type not in ('location', 'geographic'):
            raise ValueError(f"target_type must be 'location' or 'geographic', got: {target_type}")
        return target_type
    ...
    def validate_radius_miles(self, key, radius_miles):
        ...
        if radius_miles is not None and not (1 <= radius_miles <= 100):
            raise ValueError(f"radius_miles must be between 1 and 100, got: {radius_miles}")
        return radius_miles
    ...
    def format_coordinates_display(self) -> str:
        ...
        """
        if self.is_geographic_target():
            return f"{self.latitude:.5f}, {self.longitude:.5f} ({self.radius_miles} miles)"
        return ""
