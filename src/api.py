...
def match_location(location: Dict[str, Any], target_state: Optional[str], target_country: Optional[str]) -> bool:
...
async def fetch_submissions_for_coordinates(lat: float, lon: float, radius_miles: int = None, use_min_date: bool = True) -> List[Dict[str, Any]]:
...
async def fetch_submissions_for_location(location_id: int, use_min_date: bool = True) -> List[Dict[str, Any]]:
...
async def geocode_city_name(city_input: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if target_state or target_country:
            matching_results = [loc for loc in results if match_location(loc, target_state, target_country)]
            if matching_results:
