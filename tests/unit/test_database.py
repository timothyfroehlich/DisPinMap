...
import os
from src.database import Database, Base, ChannelConfig, MonitoringTarget, SeenSubmission
...
class TestDatabaseEdgeCases:
    ...
    def test_mark_duplicate_submissions_seen(self, db):
        ...
        with db.get_session() as session:
            for submission_id in submission_ids:
                count = session.query(SeenSubmission).filter_by(
    ...
    def test_channel_config_update_nonexistent(self, db):
        ...
        db.update_channel_config(channel_id, guild_id, is_active=True, poll_rate_minutes=45)

        verify_channel_config(db, channel_id, {
            'is_active': True,
    ...
    def test_channel_config_update_existing(self, db):
        ...
        db.update_channel_config(channel_id, guild_id, is_active=False, poll_rate_minutes=60)

        # Update existing config
        db.update_channel_config(channel_id, guild_id, is_active=True, notification_types='all')
...
class TestPostgreSQLSpecific:
    @pytest.mark.skipif(os.getenv('DB_TYPE') != 'postgres', reason="PostgreSQL specific test")
