...
class TestAddTargetBehavior:
    ...
    async def test_add_location_uses_min_date_false(
        self, mock_location_details, mock_fetch_submissions, monitoring_cog, db
    ):
        """
    ...
    @patch(
        "src.cogs.monitoring.fetch_submissions_for_coordinates", new_callable=AsyncMock
    )
    async def test_add_coordinates_uses_min_date_false(
        self, mock_fetch_submissions, monitoring_cog, db
    ):
        """
    ...
    async def test_add_city_uses_min_date_false(
        self, mock_geocode, mock_fetch_submissions, monitoring_cog, db
    ):
        """
        ...
        # Verify API called with correct parameters
        mock_fetch_submissions.assert_called_once_with(
            30.2672, -97.7431, radius, use_min_date=False
        )
    ...
    @patch("src.cogs.monitoring.fetch_submissions_for_location", new_callable=AsyncMock)
    @patch("src.cogs.monitoring.fetch_location_details", new_callable=AsyncMock)
    async def test_add_target_shows_exactly_five_submissions(
        self, mock_location_details, mock_fetch_submissions, monitoring_cog, db
    ):
        """
        ...
        sorted_submissions = call_args[0][
            1
        ]  # Second argument should be sorted submissions
        ...
        assert len(sorted_submissions) >= 5 or len(sorted_submissions) == len(
            submissions
        )
        ...
        for i in range(len(sorted_submissions) - 1):
            current_time = datetime.fromisoformat(
                sorted_submissions[i]["created_at"].replace("Z", "+00:00")
            )
            next_time = datetime.fromisoformat(
                sorted_submissions[i + 1]["created_at"].replace("Z", "+00:00")
            )
            assert current_time >= next_time

    @patch("src.cogs.monitoring.fetch_submissions_for_location", new_callable=AsyncMock)
    @patch("src.cogs.monitoring.fetch_location_details", new_callable=AsyncMock)
    async def test_add_target_shows_fewer_when_less_available(
        self, mock_location_details, mock_fetch_submissions, monitoring_cog, db
    ):
        """
    ...
    @patch("src.cogs.monitoring.fetch_submissions_for_location", new_callable=AsyncMock)
    @patch("src.cogs.monitoring.fetch_location_details", new_callable=AsyncMock)
    async def test_add_target_handles_api_errors_with_discord_messages(
        self, mock_location_details, mock_fetch_submissions, monitoring_cog, db
    ):
        """
    ...
    @patch("src.cogs.monitoring.fetch_submissions_for_location", new_callable=AsyncMock)
    @patch("src.cogs.monitoring.fetch_location_details", new_callable=AsyncMock)
    async def test_add_target_updates_poll_timestamps(
        self, mock_location_details, mock_fetch_submissions, monitoring_cog, db
    ):
        """
    ...
    @patch("src.cogs.monitoring.fetch_submissions_for_location", new_callable=AsyncMock)
    @patch("src.cogs.monitoring.fetch_location_details", new_callable=AsyncMock)
    async def test_add_target_activates_channel(
        self, mock_location_details, mock_fetch_submissions, monitoring_cog, db
    ):
        """
    ...
    async def test_add_target_submission_sorting(self, monitoring_cog):
        ...
        valid_submissions = [
            s for s in sorted_submissions if s.get("created_at") != "invalid-date"
        ]
