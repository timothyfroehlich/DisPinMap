...
class TestMonitorTask:
    ...
    async def test_run_checks_for_channel_location(
        self, mock_fetch, monitor, db, mock_notifier
    ):
    ...
    async def test_run_checks_for_channel_coordinates(
        self, mock_fetch, monitor, db, mock_notifier
    ):
        ...
        db.add_monitoring_target(
            channel_id, "latlong", "30.1,-97.2,10", "30.1,-97.2,10"
        )
        db.update_channel_config(channel_id, guild_id)
        config = db.get_channel_config(channel_id)
        config["last_poll_at"] = None
    ...
    @patch("src.cogs.monitor.fetch_submissions_for_location", new_callable=AsyncMock)
    async def test_run_checks_no_new_submissions(
        self, mock_fetch, monitor, db, mock_notifier
    ):
    ...
    @patch("src.cogs.monitor.fetch_submissions_for_location", new_callable=AsyncMock)
    async def test_manual_check_no_new_submissions(
        self, mock_fetch, monitor, db, mock_notifier
    ):
    ...
    @patch(
        "src.cogs.monitor.MachineMonitor._should_poll_channel", new_callable=AsyncMock
    )
    @patch(
        "src.cogs.monitor.MachineMonitor.run_checks_for_channel", new_callable=AsyncMock
    )
    async def test_monitor_task_loop(
        self, mock_run_checks, mock_should_poll, monitor, db
    ):
    ...
    async def test_run_checks_updates_last_checked_at(self, mock_fetch, monitor, db):
        ...
        time_diff = datetime.now(timezone.utc) - updated_target[
            "last_checked_at"
