...
def verify_database_target(db: Database, channel_id: int, expected_count: int, target_type: str = None):
    ...
    if target_type and expected_count > 0:
        assert all(t['target_type'] == target_type for t in targets), \
...
async def assert_discord_message(ctx, expected_message):
    ...
    assert any(expected_message in msg for msg in ctx.sent_messages), \
